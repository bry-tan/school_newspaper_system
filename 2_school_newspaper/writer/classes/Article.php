...
class Article extends Database {
    ...
    /**
     * Submits a new article with optional image
     * @param string $title The article title
     * @param string $content The article content
    ...
    public function submitArticle($title, $content, $user_id, $is_admin = 0, $image = null) {
        if ($image && $image['error'] === UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($image["tmp_name"], "../" . $target_file)) {
                $sql = "INSERT INTO articles (title, content, author_id, image_path, is_active) 
                        VALUES (?, ?, ?, ?, 1)";
                return $this->executeNonQuery($sql, [$title, $content, $user_id, $target_file]);
            }
            return false;
        } else {
            $sql = "INSERT INTO articles (title, content, author_id, is_active) 
                    VALUES (?, ?, ?, 1)";
            return $this->executeNonQuery($sql, [$title, $content, $user_id]);
        }
    }
}
?>